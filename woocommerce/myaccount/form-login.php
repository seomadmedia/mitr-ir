...
<div class="tabs woo_myaccount_login" data-toggle="tabslet" data-animation="true">
	...
	<?php if( $reg_login_joint ) : ?>
		<div>
			<div class="myaccount_title"><svg class="pack-theme" viewBox="0 0 24 24"><use xlink:href="<?= mweb_print_sprites_path() ?>#user"></use></svg> ورود و ثبت نام</div>
			<?php echo do_shortcode('[mweb_login_register]'); ?>
		</div>
	<?php else: ?>

		<ul>
			<li><a href="#login"><svg class="pack-theme" viewBox="0 0 24 24"><use xlink:href="<?= mweb_print_sprites_path() ?>#user"></use></svg> ورود</a></li>
			<li><a href="#register"><svg class="pack-theme" viewBox="0 0 24 24"><use xlink:href="<?= mweb_print_sprites_path() ?>#profile-add"></use></svg> عضویت</a></li>
		</ul>
		<div id='login'>
		...
		<div id='register'>
				<?php if ( 'yes' === get_option( 'woocommerce_enable_myaccount_registration' ) ) : ?>
					...
					<?php 
						...
						echo do_shortcode('[mweb_register]');
					} ?>

				<?php else:  ?>
					<p style="padding: 15px 20px;"><?php esc_html_e( 'امکان عضویت در سایت توسط مدیر غیر فعال شده است', 'woocommerce' ); ?></p>
				<?php endif; ?>
...
<a class="back_to_home" href="<?php echo esc_url( home_url( '/' ) ); ?>"  title="<?php bloginfo( 'name' ); ?>"><svg class="pack-theme" viewBox="0 0 24 24"><use xlink:href="<?= mweb_print_sprites_path() ?>#logout-1"></use></svg>بازگشت <span>به صفحه نخست</span></a>
