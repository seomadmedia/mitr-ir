 ...
 * @since 1.0.0
 */
class Block_Product_Slider extends Widget_Base {

	
	public function get_name() {
		return 'general-slider-product';
	}


	public function get_title() {
		return __( 'اسلایدر محصولات', 'mweb' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'only_outofstock_single',
			[
				'label' => __( 'نمایش در صفحه محصولات ناموجود', 'mweb' ),
				'type' => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
			]
		);
	...
	protected function render() {
		...
		if( $settings['only_outofstock_single'] && is_singular( 'product' ) )
			if( mweb_get_product_stock() ){
				return false;
			}else{
				$loop_arg['flag'] = 'yes';
			}
		...
		if( $settings['freemode'] == 'yes'){
			$data_setting['freeMode'] = true;
		}
		...
		$block = array('block_id' => 'mweb_'.$this->get_id(), 'block_name' => 'mweb_'.$this->get_name(), 'block_classes' => $settings['loop_type'], 'block_options' => $query_options);
