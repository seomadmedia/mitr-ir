...
?>
 <header <?php mweb_theme_schema::makeup('header'); ?>> 
	<div class="container">
		<div class="row">
			<div class="col-12 col-sm-4 col-md-3">
				<div class="logo_wrap"><?php get_template_part( 'templates/header/module', 'logo' ); ?></div> 
			</div>
			<div class="col-12 col-sm-8 col-md-9">
				<?php if ( class_exists('WooCommerce') ) : ?>
					<div class="top_icons shop_cart get_sidebar" data-class="open_cart_sidebar"> <a class="head_cart_total" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="مشاهده سبد خرید"> <span class="shop-badge header-cart-count"><?php echo sprintf (_n( '%d ', '%d ', WC()->cart->get_cart_contents_count() ), WC()->cart->get_cart_contents_count() ); ?></span><svg class="pack-theme" viewBox="0 0 24 24"><use xlink:href="<?= mweb_print_sprites_path() ?>#bag-2"></use></svg></a>
						<?php get_template_part( 'templates/header/module', 'cart' ); ?>
					</div>
				<?php endif; ?>
				<?php if(is_user_logged_in()): ?>
					<div class="top_icons user_login"><svg class="pack-theme" viewBox="0 0 24 24"><use xlink:href="<?= mweb_print_sprites_path() ?>#profile-tick"></use></svg></span>
						<?php
