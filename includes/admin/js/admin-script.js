(function($) {
	...
	$(document).ready(function ($) {
		...
		$('.add-custom-field').click(function() {
			$('.custom-field-template').clone().removeAttr('style').removeClass('custom-field-template').insertBefore('.add-custom-field');
		});
		
		$('.mw-colorpicker').wpColorPicker();
		
		$( "#_custom_last_modified" ).persianDatepicker({ 
			showGregorianDate: true,
		});
		
		$( "#_available_on" ).persianDatepicker({ 
			showGregorianDate: true,
		});
		...
		$( "input[name*='shop_options[delivery_time_holidays][]']" ).persianDatepicker({
			isRTL: 1,
			closeOnBlur: 0,
		});
		...
		const targetNode = document.getElementById('delivery_time_holidays-ul');
		const config = { childList: true, subtree: true };
		const callback = function(mutationsList, observer) {
			mutationsList.forEach(function(mutation) {
				if (mutation.type === 'childList') {
					$("input[name*='shop_options[delivery_time_holidays][]']").each(function(){
		...
		$(document).on( 'click', '.select-image', function(e) {
			...
			handle_images( frameArgs, function( selection ){
				...
				$parent.find('.mweb-image-wrap').prepend( '<img src="'+model.attributes.url+'" width="150" height="150"/>' );
		...
		function addToQueue(func, params) {
			functionQueue.queue('customQueue', function(next) {
		...
		function exampleFunction(param) {
			return new Promise(function(resolve) {
				console.log('Function execution started with parameter:', param);
				setTimeout(function() {
					console.log('Function execution completed with parameter:', param);
					load_attributes(param)
					resolve(); 
				}, 3000); 
			});
		}
		...
		load_attribute_group_category.on('click', function(e) {
			...
			$.each(attribute_groups, function() {
				addToQueue(exampleFunction, this);
			});
			
			functionQueue.queue('customQueue', function() {
				console.log('Queue ended. All functions executed.');
				//$( document.body ).find( '.woocommerce_attribute' ).find( 'h3' ).trigger('click');
			});
			
			functionQueue.dequeue('customQueue');
    	...
    	function load_attributes (attribute_group_id) {
			...
			$.ajax({
	            ...
	            success : function( response ) {
					...
					var size = $existingAttributes.length;
		...
		$( '#wcc-media-attachment' ).on('click', '.wcc-media-remove', function ( event ) {
			event.preventDefault();

			$wrap = $( this ).closest( '.wcc-media-wrap' ).remove();
			$wrap.trigger( 'wcc_media_removed' );
		});
		
		
		$( '#the-comment-list' ).on( 'click', '.wcc_media_del', function ( event ) {
			event.preventDefault();

			if ( ! confirm( 'با حذف تصاویر از بخش رسانه نیز حذف خواهد شد. موافقید ؟' ) ) {
				return;
			}
			...
			$.post( ajaxurl, data, function ( response ) {
				if ( response.success ) {
					...
					const $attachment = $comment.find( '.wcc_media_list' );
					$attachment.remove();
					$this.remove();
				}
			} );
		...
		function initializeSelect2() {
			...
			if( !$('.custom_display_features_select').length ){
				return false;
			}
			$('.custom_display_features_select').select2({
				height: '36px',
				ajax: {
					url: ajax_object.ajax_url,
					dataType: 'json',
		...
		$('#add-field').click(function() {
			var index = $('#custom_display_features_repeater .repeater-item').length;
			$('#custom_display_features_repeater').append(
				'<div class="repeater-item">' +
					'<select class="custom_display_features_select" name="product_display_features[features][' + index + '][select]" style="width: 100%;" required></select>' +
					'<input type="text" id="icon-' + index + '" name="product_display_features[features][' + index + '][icon]" placeholder="انتخاب آیکن" style="width: calc(100% - 70px)" required />' +
					'<div id="preview_icon_picker_' + index + '" data-target="#icon-' + index + '" class="button icon-picker">انتخاب</div>' +
					'<button type="button" class="remove-field button">حذف</button>' +
				'</div>'
			);
			initializeSelect2();
			$($('#preview_icon_picker_' + index)).iconPicker();
		});
		...
		$('#add-warranty').on('click', function() {
			var warrantyHtml = '<div class="warranty">' +
							   '<input type="text" name="warranty_name[]" placeholder="نام گارانتی" required />' +
							   '<input type="number" name="warranty_price[]" placeholder="قیمت" required />' +
							   '<button type="button" class="remove-warranty button">حذف</button>' +
							   '</div>';
			$('#warranties').append(warrantyHtml);
		});

		$(document).on('click', '.remove-warranty', function() {
			$(this).closest('.warranty').remove();
		});
