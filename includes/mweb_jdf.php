...
if (!function_exists('jdate')) {
	function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num_override = 'en') {

	  $T_sec = 0;/* <= رفع خطاي زمان سرور ، با اعداد '+' و '-' بر حسب ثانيه */

	  if ($time_zone != 'local') date_default_timezone_set(($time_zone === '') ? 'Asia/Tehran' : $time_zone);
	  $ts = $T_sec + (($timestamp === '') ? time() : tr_num_override($timestamp));
	  ...
	  for ($i = 0; $i < $sl; $i++) {
		...
		switch ($sub) {
		  ...
		  case 'k';
			$out .= tr_num_override(100 - (int) ($doy / ($kab + 365.24) * 1000) / 10, $tr_num_override);
			break;

		  case 'K':
			$out .= tr_num_override((int) ($doy / ($kab + 365.24) * 1000) / 10, $tr_num_override);
			break;
	  ...
	  return ($tr_num_override != 'en') ? tr_num_override($out, 'fa', '.') : $out;
	}
}

/*	F	*/
if (!function_exists('jstrftime')) {
	function jstrftime($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num_override = 'fa') {
...
if (!function_exists('jmktime')) {
	function jmktime($h = '', $m = '', $s = '', $jm = '', $jd = '', $jy = '', $none = '', $timezone = 'Asia/Tehran') {
	  ...
	  if ($h === '') {
		...
		list($h, $m, $s, $jm, $jd, $jy) = explode('_', tr_num_override($h . '_' . $m . '_' . $s . '_' . $jm . '_' . $jd . '_' . $jy));
		if ($m === '') {
		  return mktime($h);
		} else {
		  if ($s === '') {
...
if (!function_exists('jgetdate')) {
	function jgetdate($timestamp = '', $none = '', $timezone = 'Asia/Tehran', $tn = 'en') {
	  $ts = ($timestamp === '') ? time() : tr_num_override($timestamp);
	  $jdate = explode('_', jdate('F_G_i_j_l_n_s_w_Y_z', $ts, '', $timezone, $tn));
	  return array(
		'seconds' => tr_num_override((int) tr_num_override($jdate[6]), $tn),
		'minutes' => tr_num_override((int) tr_num_override($jdate[2]), $tn),
		'hours' => $jdate[1],
		'mday' => $jdate[3],
		'wday' => $jdate[7],
		'mon' => $jdate[5],
...
if (!function_exists('jcheckdate')) {
	function jcheckdate($jm, $jd, $jy) {
	  list($jm, $jd, $jy) = explode('_', tr_num_override($jm . '_' . $jd . '_' . $jy));
...
/*	F	*/
function tr_num_override($str, $mod = 'en', $mf = '٫') {
...
if (!function_exists('gregorian_to_jalali')){  
	function gregorian_to_jalali($gy, $gm, $gd, $mod = '') {
	   list($gy, $gm, $gd) = explode('_', tr_num_override($gy . '_' . $gm . '_' . $gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
...
if (!function_exists('jalali_to_gregorian')){  
	function jalali_to_gregorian($jy, $jm, $jd, $mod = '') {
	   list($jy, $jm, $jd) = explode('_', tr_num_override($jy . '_' . $jm . '_' . $jd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
	  $jy += 1595;
